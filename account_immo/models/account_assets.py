...
class AccountAssetInherit(models.Model):
    ...
    book_value = fields.Monetary(string='Valeur comptable', readonly=False, compute='_compute_book_value',
                                 recursive=True,
                                 store=True,
    ...
    def set_to_draft(self):
        for rec in self:
            rec.write({'state': 'draft', 'cession_value': 0, 'plus_value': 0, 'moins_value': 0,
                       'amort_total': rec.already_depreciated_amount_import})
    ...
    def set_to_close(self, invoice_line_ids, date=None, message=None, invoice_ids=None, loss_account_id=None):
        ...
        move_ids = full_asset._get_disposal_moves([invoice_line_ids] * len(full_asset), disposal_date, invoice_ids,
                                                  loss_account_id)
    ...
    def _create_move_before_date(self, date, loss_account_id):
        ...
        days_depreciated, amount = self._compute_board_amount(value_residual, beginning_depreciation_date, date,
                                                              days_already_depreciated, days_left, value_residual)
        ...
        if not float_is_zero(amount, precision_rounding=self.currency_id.rounding):
            ...
            if posted_depreciation_moves:
                ...
                new_line = self._insert_depreciation_line(
                    self.original_value - amount - self.already_depreciated_amount_import, beginning_depreciation_date,
                    date, days_depreciated, loss_account_id)
                new_line._post()
            ...
            for asset in self:
                if imported_amount == 0:
                    ...
                    if posted_depreciation_moves:
                        ...
                        asset.write(
                            {'book_value': self.original_value - amount - self.already_depreciated_amount_import,
                             'value_residual': self.original_value - amount - self.already_depreciated_amount_import})
    ...
    def _get_disposal_moves(self, invoice_lines_list, disposal_date, invoice_ids, loss_account_id):
        ...
        for asset, invoice_line_ids in zip(self, invoice_lines_list):
            ...
            posted_depreciation_moves = self.env['account.move'].search(
                [('asset_id', '=', asset.id), ('journal_id', '=', asset.disposal_journal_id.id),
                 ('state', '=', 'posted')])
            ...
            asset.write({'amort_total': amount_depre - asset.original_value, 'cession_date': disposal_date,
                         'cession_value': amount_invoiceline})
    ...
    def set_to_running(self):
        for rec in self:
            if rec.value_residual == 0 or rec.method_number == 0:
                ...
                try:
                    if rec.depreciation_move_ids and not max(rec.depreciation_move_ids,
                                                             key=lambda m: (m.date, m.id)).asset_remaining_value == 0:
